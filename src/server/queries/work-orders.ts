...
export async function listWorkOrders(opts?: { status?: string; clientId?: string; search?: string; completedAndPaid?: boolean }) {
  const conditions = [];
  ...
  if (opts?.completedAndPaid !== undefined) {
    if (opts.completedAndPaid) {
      // Only items that are BOTH completed AND paid
      conditions.push(eq(workOrders.isCompleted, true));
      conditions.push(eq(workOrders.isPaid, true));
    }
  }
  if (opts?.search) {
    const q = `%${opts.search}%`;
